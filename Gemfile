# frozen_string_literal: true

source "https://rubygems.org"

# Specify your gem's dependencies in ml-experiment.gemspec
gemspec

gem "rake", "~> 13.0"

gem "minitest", "~> 5.0"

gem "rubocop", "~> 1.21"

gem "svg-graph"

gem "rubystats"

gem "ml-forest", git: "https://github.com/forest-implementation/ml-forest"
# gem "ml-forest",:path => "/home/adamo/source/rubyml/ml-forest"

gem "ml-service-isolation", github: "forest-implementation/ml-service-isolation", branch: "range_as_key"

# gem "ml-service-isolation", :path => "/home/adamo/scripts/ruby-forest-test/ml-service-isolation"

gem "enumerable-statistics", "~> 2.0"

gem "rinruby", github: "clbustos/rinruby", ref: "fa04e5fd5048445b0aa49786b6cba41724251673"

gem 'sorbet', :group => :development

gem 'sorbet-runtime'

gem 'tapioca', require: false, :group => :development

gem "gnuplot", "~> 2.6"

gem "ruby-graphviz", "~> 1.2"
