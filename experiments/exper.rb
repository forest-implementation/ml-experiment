...
trainData = CSV.readlines("data/clicked/csv/data_novelties_clicked_train.csv", headers: false,
                                                                               converters: %i[numeric]).map do |x|
  [x[0], x[1]]
end

testData = CSV.readlines("data/clicked/csv/data_novelties_clicked_test.csv", headers: false,
                                                                             converters: %i[numeric]).map do |x|
  [x[0], x[1]]
end
