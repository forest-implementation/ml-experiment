...
module Stats
  module Statistics
    ...
    def column_deviation(input, index)
      ...
      fqr = quantil(sorted, 0.25)
      tqr = quantil(sorted, 0.75)
      iqr = tqr - fqr
    ...
    def greater_h(kernel_h, p, q, u); end

    def less_h(kernel_h, p, q, u); end
    ...
    def medcouple_iteration(left, right, r_total: 0, l_total: 0, p: 0, q: 0, zplus: [], zminus: [])
      ...
      weights = middle_idx.map { |_x| right[i] - left[i] + 1 }
      weighted_median = weighted_median(row_medians, weights)
      # TODO: continue here
    end

    def fast_medcouple(_arr)
      zplus, zminus = divide_z(sorted_arr)
      p = zplus.size
      q = zminus.size
      left = Array.new(p, 0)
